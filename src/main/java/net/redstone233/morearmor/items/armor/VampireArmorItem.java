...
public class VampireArmorItem extends ArmorItem {
    private static final Map<ArmorMaterial, List<StatusEffectInstance>> MAP =
            (new ImmutableMap.Builder<ArmorMaterial,List<StatusEffectInstance>>())
                    .put(ModArmorMaterials.VAMPIRE,
                            Arrays.asList(
                                    new StatusEffectInstance(StatusEffects.GLOWING,1000,1,false,false,true),
                                    new StatusEffectInstance(StatusEffects.REGENERATION,100,4,false,false,true),
                                    new StatusEffectInstance(StatusEffects.RESISTANCE,1000,4,false,false,true),
                                    new StatusEffectInstance(StatusEffects.HEALTH_BOOST,100,4,false,false,false)
                            )).build();

    public VampireArmorItem(ArmorMaterial material, EquipmentType type, Settings settings) {
    ...
    private boolean hasCorrectArmorSet(PlayerEntity player, ArmorMaterial material) {
        ...
        if (!isCustomArmor(player.getInventory().getArmorStack(3))) {
            return false;
        } else if (!isCustomArmor(player.getInventory().getArmorStack(2))) {
            return false;
        } else if (!isCustomArmor(player.getInventory().getArmorStack(1))) {
            return false;
        } else if (!isCustomArmor(player.getInventory().getArmorStack(0))) {
            return false;
        } else {
            return true;
        }
